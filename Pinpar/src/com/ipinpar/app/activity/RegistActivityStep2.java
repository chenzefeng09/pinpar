	...
	protected void onCreate(Bundle arg0) {
					...
					public void onResponse(JSONObject response) {
						...
						try {
							if (response.getInt("result")  == 1) {
								...
								Toast.makeText(mContext, "获取验证码失败，请重试"+response.getInt("result") , 1000).show();
							}
		...
		btn_next.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				email_address = et_email.getText().toString().trim();
				...
				VerifyCodeRequest request = new VerifyCodeRequest(phone, identifier, verify_code, new Listener<JSONObject>() {
					...
					public void onResponse(JSONObject response) {
						...
						try {
							if (response.getInt("result") == 1) {
								startActivity(RegistStep3Activity.getIntent2Me(mContext, phone, pwd, email_address));
							}
							else {
