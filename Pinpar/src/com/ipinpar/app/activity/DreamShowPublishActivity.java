...
public class DreamShowPublishActivity extends PPBaseActivity {
	...
	private EditText et_add_text,et_add_detail;
	...
	protected void onCreate(Bundle arg0) {
		...
		et_add_detail = (EditText) findViewById(R.id.et_add_detail);
		...
		iv_add_img.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				// TODO Auto-generated method stub
				new AlertDialog.Builder(DreamShowPublishActivity.this)
				.setTitle("请选择图片")
		...
		btn_save.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				showProgressDialog();
				try {
					DreamShowPublishRequest request = new DreamShowPublishRequest(UserManager.getInstance().getUserInfo().getUid()
							, et_add_text.getText().toString().trim(), et_add_detail.getText().toString().trim(), uploadUrl, new Listener<JSONObject>() {
								...
								public void onResponse(JSONObject response) {
									...
									try {
										if (response != null && response.getInt("result") == 1) {
											...
											finish();
											Toast.makeText(mContext, "发布梦想秀成功！", 1000).show();

										}
										else {
											Toast.makeText(mContext, "发布梦想秀出错，请稍后重试", 1000).show();
										}
